...
namespace Sinoo.Common
{
    ...
    public class ImportHelper
    {
        ...
        /// xml集合
        /// </summary>
        private Dictionary<string, XmlHandler> _dicXmls = new Dictionary<string, XmlHandler>();
        /// <summary>
        /// 获取到的全部数据
        /// </summary>
        /// <summary>
        ...
        public ImportHelper(string connectionString)
        {
            ...
            XmlHandler xmlRegex = new XmlHandler("RegexConfig.xml");//读取正则配置文件
            if (xmlRegex != null)
            {
        ...
        public DataSet ValidateData(string xmlPath, string mainTableName, bool isDeleteExists)
        {
            ...
            {
                XmlHandler xmlHandler = new XmlHandler(string.Format(@"{0}\{1}.xml", xmlPath, this._allData.Tables[i].TableName));
                _dicXmls.Add(this._allData.Tables[i].TableName, new XmlHandler(String.Format("{0}\\{1}.xml", xmlPath, this._allData.Tables[i].TableName)));      //获取XMl
                DataTable dtXml = xmlHandler.GetAllData();
                TableValidate validate = GetTableValidate(xmlHandler, dtXml, this._allData.Tables[i]);
            ...
            if (_dicValidate.Count > 0 && _dicValidate.Keys.Contains(mainTableName))
            {
                ...
                if (this._errorInfo.Rows.Count == 0)
                {
                    for (int i = 0; i < this._allData.Tables.Count; i++)
                    {
                        ...
                        Dictionary<string, string> dicAutoFill = GetDicAutoFill(dtXml);//获取xml配置表中所有需要验证正则的数据 <"样本数据","001">
                        ...
                        //如果有需要进行
                        if (dicAutoFill.Count > 0)
                        {
                            for (int j = 0; j < dicAutoFill.Count; j++)
                            {
                                string AutoFillInnerText = dicAutoFill.Keys.ElementAt(j);
                                string AutoFillValue = dicAutoFill.Values.ElementAt(j);
                                if (this._allData.Tables[0].Columns.Contains(AutoFillInnerText))
                                {
                                    this._allData.Tables[0].Columns.Remove(AutoFillInnerText);
                                }
                                    if (AutoFillValue == "Auto_TimeNow")
                                    {
                                        this._allData.Tables[i].Columns.Add(new DataColumn() { ColumnName = AutoFillInnerText, DefaultValue = DateTime.Now });
                                    }
                                    else if (AutoFillValue == "Auto_GUID")
                                    {
                                        this._allData.Tables[0].Columns.Add(new DataColumn() { ColumnName = AutoFillInnerText});
                                        if (this._allData!=null&&this._allData.Tables[0].Rows.Count>0)
                                        {
                                            for (int z = 0; z < this._allData.Tables[0].Rows.Count; z++)
                                            {
                                                this._allData.Tables[0].Rows[z][AutoFillInnerText] = Guid.NewGuid();
                                            }
                                        }
                                    }
                                    else if (AutoFillValue != "")
                                    {
                                        this._allData.Tables[0].Columns.Add(new DataColumn() { ColumnName = AutoFillInnerText, DefaultValue = AutoFillValue });
                                    }
                            }
                        }
                    }
        ...
        /// <returns></returns>
        private TableValidate GetTableValidate(XmlHandler xmlHandler, DataTable dtXml, DataTable sourceDataTable)
        {
            string pkName = xmlHandler.GetRootNodeAttr("pk");
            string fkName = xmlHandler.GetRootNodeAttr("fk");
            string dbTableName = xmlHandler.GetRootNodeAttr("tableName");
        ...
        private Dictionary<string, string> GetDicAutoFill(DataTable dtXmlData)
        {
            var dic = new Dictionary<string, string>();
            for (int i = 0; i < dtXmlData.Rows.Count; i++)
            {
                if (dtXmlData.Rows[i]["AutoFill"] != null && !string.IsNullOrEmpty(dtXmlData.Rows[i]["AutoFill"].ToString()))
                {
                    dic.Add(dtXmlData.Rows[i]["cn"].ToString(), dtXmlData.Rows[i]["AutoFill"].ToString());
                }
            }
            return dic;
        }
        ...
        private Dictionary<string, Dictionary<string, string>> GetDicXmlPathData(DataTable dtXmlData)
        {
            ...
            {
                if (
                    //(dtXmlData.Rows[i]["valid"] != null && Convert.ToBoolean(dtXmlData.Rows[i]["valid"]))
                    //&& 
                    (dtXmlData.Rows[i]["path"] != null && !string.IsNullOrWhiteSpace(dtXmlData.Rows[i]["path"].ToString())))
                {
                    xmlHandler = new XmlHandler(dtXmlData.Rows[i]["path"].ToString());
                    dtPathXml = xmlHandler.GetAllData();
