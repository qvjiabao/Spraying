...
namespace Sinoo.Spraying.Page.ReportManagement.DailyReport
{
    ...
    public partial class DaliyCollection : BasePage
    {
        ...
        private string GetSqlWhere()
        {
            ...
            string sqlwhere2 = @" ( CASE WHEN OP01014 <> '' THEN OP01014
                                       WHEN OP01012 <> '' THEN OP01012
                                       WHEN OP01010 <> '' THEN OP01010
                                       WHEN OP01008 <> '' THEN OP01008
                                       ELSE NULL
                                  END ) ";
            ...
            if (!string.IsNullOrEmpty(this.txtOP01DateEnd.Text.Trim()))
            {
                sqlWhere += string.Format(" AND {0} <= '{1}'", sqlwhere2, this.txtOP01DateEnd.Text.Trim());
            }
        ...
        private void InitData(int Index, bool bl)
        {
            ...
            DataTable dt = _ReportBLL.SelectDaliyCollection(AspNetPager1.PageSize, Index, strSqlWhere, ref RowsCount);
            if (dt.Rows.Count > 0)
            {
            ...
            else
            {
                if (bl)
                {
                    DataCount = "<tr><td colspan=\"14\">系统暂无数据显示</td></tr>";
                }
                else
                {
                    DataCount = "<tr><td colspan=\"14\">没有符合查询条件的数据</td></tr>";
                }
            }
            this.repDaliyCollection.DataSource = dt;
            ...
            this.repDaliyCollection.DataBind();
        }
        ...
        {
            DataSet ds = _ExcelBLL.ExportDaliyCollection(GetSqlWhere());
            if (ds.Tables[0].Rows.Count > 0)
            {
                this.DownloadFile(ds, "DaliyCollection.xls", "DaliyCollection.xml");
            }
            else
            {
