...
namespace Sinoo.Spraying.Page.ReportManagement.MonthlyReport
{
    public partial class KeyCustomer : System.Web.UI.Page
    {
        ...
        private void InitData(int Index, bool bl)
        {
            ...
            DataTable dt = _ReportBLL.SelectKeyCustomer(AspNetPager1.PageSize, Index, strWhere, ref RowCount);
            if (dt.Rows.Count > 0)
            {
        ...
        {
            DataSet ds = _ExcelBLL.ExportKeyCustomer(GetSqlWhere());
            if (ds.Tables[0].Rows.Count > 0)
            {
                this.DownloadFile(ds, "KeyCustomer.xls", "KeyCustomer.xml");
            }
            else
            {
