...
{
    public partial class HotPartList : System.Web.UI.Page
    {
        ...
        private void InitData(int Index, bool bl)
        {
            ...
            DataTable dt = _ReportBLL.SelectMonthlyHotPartListPage(AspNetPager1.PageSize, Index, strWhere,ref RowCount);
            if (dt.Rows.Count > 0)
            {
            ...
            else
            {
                if (bl)
                {
                    DataCount = "<tr><td colspan=\"2\">系统暂无数据显示</td></tr>";
                }
                else
                {
                    DataCount = "<tr><td colspan=\"2\">没有符合查询条件的数据</td></tr>";
                }
            }
            repHotPart.DataSource = dt;
        ...
        {
            DataSet ds = _ExcelBLL.ExportHotPartList(GetSqlWhere());
            if (ds.Tables[0].Rows.Count > 0)
            {
                this.DownloadFile(ds, "HotPartList.xls", "PA02.xml");
            }
            else
            {
