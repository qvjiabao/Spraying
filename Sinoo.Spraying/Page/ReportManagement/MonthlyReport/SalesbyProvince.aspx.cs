...
{
    public partial class SalesbyProvince : System.Web.UI.Page
    {
        ...
        private void InitData(int Index, bool bl)
        {
            ...
            DataTable dt = _ReportBLL.SelectMonthlyProvincePage(AspNetPager1.PageSize, Index, strWhere,blInvoice, ref RowCount);
            if (dt.Rows.Count > 0)
            {
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            //开发票查询特殊处理
            bool blInvoice = (!string.IsNullOrEmpty(this.txtBeginInvoiceTime.Text.Trim()) || !string.IsNullOrEmpty(this.txtEndInvoiceTime.Text.Trim()));
            DataSet ds = _ExcelBLL.ExportSalesbyProvince(GetSqlWhere(), blInvoice);
            if (ds.Tables[0].Rows.Count > 0)
            {
                this.DownloadFile(ds, "SalebyProvince.xls", "SalebyProvince.xml");
            }
            else
            {
