...
{
    public partial class DormantCustomersTwo : System.Web.UI.Page
    {
        ...
        private void InitData(int Index, bool bl)
        {
            ...
            DataTable dt = _ReportBLL.SelectDormantCustomersTwoPage(AspNetPager1.PageSize, Index, strWhere, blInvoice, ref RowCount);
            if (dt.Rows.Count > 0)
            {
            ...
            else
            {
                if (bl)
                {
                    DataCount = "<tr><td colspan=\"5\">系统暂无数据显示</td></tr>";
                }
                else
                {
                    DataCount = "<tr><td colspan=\"5\">没有符合查询条件的数据</td></tr>";
                }
            }
        ...
        {
            string strWhereAdd = this.GetSqlWhere(); //获取查询条件            
            bool blInvoice = (!string.IsNullOrEmpty(this.txtBeginInvoiceTime.Text.Trim()) || !string.IsNullOrEmpty(this.txtEndInvoiceTime.Text.Trim()));
            DataSet dt = _ExcelBLL.ExportDormantCustomersTwoPage(strWhereAdd,blInvoice);
            if (dt.Tables[0].Rows.Count > 0)
            {
                this.DownloadFile(dt, "DormantCustomers2.xls", "DormantCustomersTwo.xml");
            }
            else
            {
