...
{
    public partial class SalesbyCity : System.Web.UI.Page
    {
        ...
        private void InitData(int Index,bool bl)
        {
            ...
            DataTable dt = _ReportBLL.SelectMonthlyCityPage(AspNetPager1.PageSize, Index, strWhere,blInvoice, ref RowCount);
            if (dt.Rows.Count > 0)
            {
        ...
        {
            bool blInvoice = (!string.IsNullOrEmpty(this.txtBeginInvoiceTime.Text.Trim()) || !string.IsNullOrEmpty(this.txtEndInvoiceTime.Text.Trim()));
            DataSet ds = _ExcelBLL.ExportsalesbyCity(GetSqlWhere(), blInvoice);
            if (ds.Tables[0].Rows.Count > 0)
            {
                this.DownloadFile(ds, "SalesbyCity.xls", "SalesbyCity.xml");
            }
            else
            {
