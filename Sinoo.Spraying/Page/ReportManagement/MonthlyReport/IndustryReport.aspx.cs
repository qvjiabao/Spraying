...
{
    public partial class IndustryReport : System.Web.UI.Page
    {
        ...
        private void InitData(int Index, bool bl)
        {
            ...
            string strWhereAdd = this.GetWhere(bl); //Where查询
            ...
            DataTable dt = _ReportBLL.SelectIndustryReportPage(AspNetPager1.PageSize, Index, strWhereAdd, blInvoice,ref RowCount);
            if (dt.Rows.Count > 0)
            {
            ...
            else
            {
                if (bl)
                {
                    DataCount = "<tr><td colspan=\"15\">系统暂无数据显示</td></tr>";
                }
                else
                {
                    DataCount = "<tr><td colspan=\"15\">没有符合查询条件的数据</td></tr>";
                }
            }
        ...
        protected void btnExPort_Click(object sender, EventArgs e)
        {
            ...
            bool blInvoice = (!string.IsNullOrEmpty(this.txtBeginInvoiceTime.Text.Trim()) || !string.IsNullOrEmpty(this.txtEndInvoiceTime.Text.Trim()));
            DataSet dt = _ExcelBLL.ExportIndustryReportPage(strWhereAdd, blInvoice);
            if (dt.Tables[0].Rows.Count > 0)
            {
                this.DownloadFile(dt, "IndustryReport.xls", "IndustryReport.xml");
            }
            else
            {
