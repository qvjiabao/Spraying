...
namespace Sinoo.Spraying.Page.ReportManagement.MonthlyReport
{
    public partial class ByCustomerName : BasePage
    {
        ...
        public string GetSqlWhere()
        {
            ...
            if (!string.IsNullOrEmpty(this.txtCA01003.Text.Trim()))
            {
                strWhereAdd += string.Format(" AND CA01003 like '%{0}%' ", txtCA01003.Text.Trim());
            }
        ...
        private void InitData(int Index, bool bl)
        {
            ...
            else
            {
                if (bl)
                {
                    DataCount = "<tr><td colspan=\"4\">系统暂无数据显示</td></tr>";
                }
                else
                {
                    DataCount = "<tr><td colspan=\"4\">没有符合查询条件的数据</td></tr>";
                }
            }
            repByCustomerName.DataSource = dt;
            AspNetPager1.RecordCount = Convert.ToInt32(RowCount);
            AspNetPager1.CurrentPageIndex = Index;

            repByCustomerName.DataBind();
        }
        ...
        {
            bool blInvoice = (!string.IsNullOrEmpty(this.txtBeginInvoiceTime.Text.Trim()) || !string.IsNullOrEmpty(this.txtEndInvoiceTime.Text.Trim()));
            DataSet ds = _ExcelBLL.ExportByCustomerName(GetSqlWhere(), blInvoice);
            if (ds.Tables[0].Rows.Count > 0)
            {
                this.DownloadFile(ds, "ByCustomerName.xls", "ByCustomerName.xml");
            }
            else
            {
