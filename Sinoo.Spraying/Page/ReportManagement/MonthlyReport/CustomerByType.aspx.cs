...
{
    public partial class CustomerByType : System.Web.UI.Page
    {
        ...
        private void InitData(int Index, bool bl)
        {
            ...
            DataTable dt = _ReportBLL.SelectMonthlyByCustomerTypePage(AspNetPager1.PageSize, Index, strWhere,blInvoice, ref RowCount);
            if (dt.Rows.Count > 0)
            {
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            //开发票查询特殊除处理
            bool blInvoice = (!string.IsNullOrEmpty(this.txtBeginInvoiceTime.Text.Trim()) || !string.IsNullOrEmpty(this.txtEndInvoiceTime.Text.Trim()));
            DataSet ds = _ExcelBLL.ExportByCustomerType(GetSqlWhere(),blInvoice);
            if (ds.Tables[0].Rows.Count > 0)
            {
                this.DownloadFile(ds, "ByCustomerType.xls", "CB01.xml");
            }
            else
            {
