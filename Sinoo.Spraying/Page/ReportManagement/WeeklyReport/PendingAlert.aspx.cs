...
{
    public partial class PendingAlert : System.Web.UI.Page
    {
        ...
        private void InitData(int Index, bool bl)
        {
            ...
            string strWhereAdd = this.GetWhere(bl); //查询Where条件
            ...
            DataTable dt = _ReportBLL.SelectPendingAlertPage(AspNetPager1.PageSize, Index, strWhereAdd,blInvoice, ref RowCount);
            if (dt.Rows.Count > 0)
            {
        ...
        private string GetWhere(bool bl)
        {
            ...
            {
                strWhereAdd += string.Format(" AND OA01009 >= '{0}'", txtBeginTime.Text.Trim());  //订单时间起始
            }
            if (!string.IsNullOrEmpty(this.txtEndTime.Text.Trim()))
            {
                strWhereAdd += string.Format(" AND OA01009 <= '{0}'", txtEndTime.Text.Trim());  //订单时间结果
            }
        ...
        protected void btnExPort_Click(object sender, EventArgs e)
        {
            ...
            DataSet dt = _ExcelBLL.ExportPendingAlertPage(strWhereAdd);
            if (dt.Tables[0].Rows.Count > 0)
            {
                this.DownloadFile(dt, "PendingAlert.xls", "PendingAlert.xml");
            }
            else
            {
